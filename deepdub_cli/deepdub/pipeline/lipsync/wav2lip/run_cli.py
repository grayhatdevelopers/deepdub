...
sys.path.append(parent_dir_name + "/wav2lip")
# ------ RELATIVE-FIX END
...
def run(
	...
	for idx, (translated_audio_path, extracted_video_path) in enumerate(zip(translated_audio_paths, extracted_video_paths)):
		...
		if not os.path.isfile(extracted_video_path):
			raise ValueError('--face argument must be a valid path to video/image file')

		elif extracted_video_path.split('.')[1] in ['jpg', 'png', 'jpeg']:
			full_frames = [cv2.imread(extracted_video_path)]
			fps = args.fps

		else:
			video_stream = cv2.VideoCapture(extracted_video_path)
		...
		translated_video_paths.append (final_output_filepath)
